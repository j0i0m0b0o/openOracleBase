...
contract openOracleBounty is ReentrancyGuard {
    ...
    function createOracleBounty(uint256 reportId, uint256 bountyStartAmt, address creator, address editor, uint16 bountyMultiplier, uint16 maxRounds, bool timeType, uint256 start) external payable nonReentrant {
        _createOracleBounty(reportId, bountyStartAmt, creator, editor, bountyMultiplier, maxRounds, timeType, start, 0);
    }

    // overload to automatically set start to current block number or time
    // see _createOracleBounty for NatSpec
    function createOracleBounty(uint256 reportId, uint256 bountyStartAmt, address creator, address editor, uint16 bountyMultiplier, uint16 maxRounds, bool timeType) external payable nonReentrant {
    ...
    // see _createOracleBounty for NatSpec
    function createOracleBountyFwd(uint256 reportId, uint256 bountyStartAmt, address creator, address editor, uint16 bountyMultiplier, uint16 maxRounds, bool timeType, uint256 forwardStartTime) external payable nonReentrant {
        uint256 start = timeType ? block.timestamp : block.number;
        start += forwardStartTime;
        _createOracleBounty(reportId, bountyStartAmt, creator, editor, bountyMultiplier, maxRounds, timeType, start, forwardStartTime);
    }
     ...
     */
    function _createOracleBounty(uint256 reportId, uint256 bountyStartAmt, address creator, address editor, uint16 bountyMultiplier, uint16 maxRounds, bool timeType, uint256 start, uint256 forwardStartTime) internal {

        if (maxRounds == 0 || bountyStartAmt == 0 || bountyMultiplier == 0 || msg.value == 0) revert InvalidInput("amounts cannot = 0");
        if (Bounty[reportId].maxRounds > 0) revert InvalidInput("reportId has bounty");        
        if (bountyStartAmt > msg.value) revert InvalidInput("msg.value too low");
        ...
        Bounty[reportId].totalAmtDeposited = msg.value;
        ...
        emit BountyCreated(reportId, msg.value, bountyStartAmt, maxRounds, bountyMultiplier, block.timestamp, timeType, start);
    }
    /**
    ...
    function recallBounty(uint256 reportId) external nonReentrant {
        ...
        if (bounty.claimed) {
            ...
            emit BountyRecalled(reportId, amount);
        } else {
            bounty.recalled = true;
            _sendEth(bounty.creator, bounty.totalAmtDeposited);
            emit BountyRecalled(reportId, bounty.totalAmtDeposited);
        }
    ...
    function editBounty(uint256 reportId, uint256 newReportId) external nonReentrant {
        ...
        if (bounty.claimed){
            ...
            emit BountyRetargeted(newReportId, reportId, amount, Bounty[newReportId].bountyStartAmt, bounty.maxRounds, bounty.bountyMultiplier, block.timestamp, bounty.timeType, Bounty[newReportId].start);
    }
    ...
    function _submitInitialReport(uint256 reportId, uint256 amount1, uint256 amount2, bytes32 stateHash, address reporter) internal {
        ...
        uint256 bountyAmt = calcBounty(bounty.start, bounty.bountyStartAmt, bounty.maxRounds, bounty.bountyMultiplier, bounty.totalAmtDeposited, bounty.timeType);
        ...
        emit BountyInitialReportSubmitted(reportId, bountyAmt);
    }

    function calcBounty(uint256 start, uint256 bountyStartAmt, uint256 maxRounds, uint256 bountyMultiplier, uint256 totalAmtDeposited, bool timeType) internal view returns (uint256){

        uint256 currentTime = timeType ? block.timestamp : block.number;
        if (currentTime < start) revert InvalidInput("start time");

            uint256 rounds = (currentTime - start);
            if (rounds > maxRounds) {
                rounds = maxRounds;
            }
